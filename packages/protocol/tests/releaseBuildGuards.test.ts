...
describe("mobile production send path source", () => {
  ...
  it("release builds gate debug BLE ping, diagnostics, and secret fallback", () => {
    ...
    const unguardedSettings = settings.replace(/\{__DEV__ \? \([\s\S]*?\) : null\}/g, "");
    expect(unguardedSettings).not.toContain("/device-diagnostics");
    expect(unguardedSettings).not.toContain("/ble-debug");
    expect(unguardedSettings).not.toMatch(/buttonLabel[\s\S]*Replace local identity keys/);
